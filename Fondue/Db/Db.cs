...
namespace Fondue
{
    ...
    public class Db
    {
        ...
        {
            return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "fondue.db");
        }
        ...
        {
            // Create
            var provider = DataProtectionProvider.Create("fondue");
            m_protector = provider.CreateProtector("fondue.db");
                                         ...
                                         bool supporters = false)
        {
            ...
            {
                QuickRun(conn,"""
                      DROP TABLE IF EXISTS `fon_config`;
                      CREATE TABLE `fon_config` (
                      ...
                      INSERT INTO `fon_config` (id) VALUES ('1');
                      """);
            }
            ...
            if (players)
            {
                QuickRun(conn,
                    """
                    DROP TABLE IF EXISTS `fon_players`;
                    CREATE TABLE `fon_players` (
                    ...
                    ALTER TABLE `fon_players` AUTO_INCREMENT=10000000;
                    """);
            }
            ...
            {
                QuickRun(conn, """
                   DROP TABLE IF EXISTS `fon_sessions`;
                   CREATE TABLE `fon_sessions` (
