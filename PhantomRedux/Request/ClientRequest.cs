    ...
    public class ClientRequest<T> where T : CommonRequest
    {
        ...
        public ClientRequest(MySqlConnection conn, string post, bool ignore_session = false)
        {
            ...
            if (!ignore_session)
            {
                ...
                var sql = Db.GetCommand("SELECT expiry FROM `pha_sessions` WHERE sid = '{0}';", deserial.common.session_id);
                ...
                else
                {
                    ...
                    if (expiry < DateTimeOffset.Now.ToUnixTimeSeconds())
                    {
                        ...
                        sql = Db.GetCommand("DELETE FROM `pha_sessions` WHERE sid = '{0}';",
                    ...
                    {
                        sql = Db.GetCommand("SELECT uid FROM `pha_sessions` WHERE sid = '{0}';",
                            deserial.common.session_id);
                        command = new MySqlCommand(sql, conn);
